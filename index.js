...
function processLastItem(stringList, callback) {
  return callback(stringList[stringList.length-1]); //how come you need to put in strong brackets?
}
...
function processSum(numberList, callback) {
  return callback(numberList.reduce(function(accum, currentValue){
...
function getFullNames(runners){
  ...
  runners.forEach(function(runners){
    return fullName.push(`${runners.last_name}, ${runners.first_name}`);
  });
  return fullName;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  let shirt = runners.filter(function(size){
...
function tallyUpDonations(runners) {
  let sum = runners.reduce(function(accum, currentValue){
